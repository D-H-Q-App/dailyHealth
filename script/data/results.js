module.exports = [
  {
    recommendation:
      "Drink more warm water or decaffinated tea, Add a little more ghee or olive oil to your diet, Enjoy some daily Licorice Tea",
    questionId: 1,
  },
  {
    recommendation:
      "Enjoy more cheese, yogurt, black lentils, cream of wheat, ripe mangos or bananas, red meat, and if its summer time, a little ice cream at lunch time!",
    questionId: 2,
  },
  {
    recommendation:
      "Enjoy warm liquids or teas only, add fresh ginger, garlic, and black pepper to your diet",
    questionId: 3,
  },
  {
    recommendation:
      "Add more ghee, olive oil, and  warm spiced milks to your diet, as well as self oil massages.",
    questionId: 4,
  },
  {
    recommendation:
      "Add more meat to your diet if you can, cream of wheat, and warm wheat breads",
    questionId: 5,
  },
  {
    recommendation:
      "Add more meat to your diet if you can, cottage cheese, and cashews.",
    questionId: 6,
  },
  {
    recommendation:
      "Enjoyed sauteed okra, cooked black lentils, and  Psyllium husk bread",
    questionId: 7,
  },
  {
    recommendation:
      "Add more quinoa, barley, popcorn, sauteed bitter melon, and Tulsi Tea to your diet. Avoid heavy foods like sweets, beef, and pork.",
    questionId: 8,
  },
  {
    recommendation:
      "Enjoy only warm liquids. Have a daily ginger tea. Add fresh ginger, garlic, and black pepper to your diet.",
    questionId: 9,
  },
  {
    recommendation:
      "Add more cooked green vegetables to your diet, like kale, celery, green beans, etc. Include dry ginger and black pepper in your cooking. Enjoy low-sodium and lightly buttered popcorn, unsweetened puffed rice or puffed barley when you need a snack. Avoid heavy foods like sweets, beef, and pork.",
    questionId: 10,
  },
  {
    recommendation:
      "Enjoy more cooked green vegtables like kale, celery, arugala, green beans, sprouts etc. Add more cumin and all spice to your diet and enjoy chia tea in the morning.",
    questionId: 11,
  },
  {
    recommendation: "Add a little pumpkin and sunflower seeds to your diet.",
    questionId: 12,
  },
  {
    recommendation:
      "Add more quinoa, barley, healthy popcorn, sauteed bitter melon, cooked kale, and Tulsi Tea to your diet. And avoid heavy foods like beef, pork, or sweets.",
    questionId: 13,
  },
  {
    recommendation:
      "Enjoy some dry toast, cooked millet or barley, sauteed bitter melon, and Tulsi tea",
    questionId: 14,
  },
  {
    recommendation: "Enjoy a little more warm milk or cream in your diet.",
    questionId: 15,
  },
  {
    recommendation:
      "Enjoy a little room temperture or, if excessively hot, iced water. Add wheat, barley, cucumber, and coriander into your diet.",
    questionId: 16,
  },
  {
    recommendation:
      "Enjoy more cheese, yogurt, black lentils, cream of wheat, ripe mangos or bananas, red meat, and if its summer time, a little ice cream at lunch time!",
    questionId: 17,
  },
  {
    recommendation:
      "Avoid acidic foods like coffee or overly spiced foods. Add more warm milks or cream to your diet.",
    questionId: 18,
  },
  {
    recommendation:
      "Add more meats and wheat products to your diet if you can. Cream of wheat is a great option in the mornings!",
    questionId: 19,
  },
  {
    recommendation:
      "Add a little more ghee, olive oil, and yogurt to your diet.",
    questionId: 20,
  },
];
